...
class CarOffloadEnv:
    ...
    def __init__(self, num_agents=1, min_task_size=10, max_task_size=100,
        ...
        # 使用连续动作空间
        self.action_space = ActionSpace(low=[0], high=[1])  # 连续空间
        self.state_space = (num_agents, 3)
        self.num_agents = num_agents
        self.min_task_size = min_task_size
        self.max_task_size = max_task_size
        self.process_rsu = process_rsu
        self.process_car = process_car
    ...
    def get_action_size(self):
        """获取动作空间的大小"""
        return self.action_space.low.shape[0]
    ...
    def step(self, action):
        ...
        for i in range(self.num_agents):
            T_local = 0
            T_offload = 0
            agent_action = action[i]  # 获取第 i 个智能体的动作

            if 0 <= agent_action <= 0.5:     # 本地处理(换成连续动作空间的形式)
                T_local = self.state[i, 1] / self.process_car
                self.state[i, 2] = 1
            elif 0.5 < agent_action <= 1:   # 卸载(换成连续动作空间的形式)
